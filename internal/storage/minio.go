...
func (m *MinioClient) PutObject(ctx context.Context, bucket, objectName string, reader io.Reader, objectSize int64, contentType string, userMetadata map[string]string) (minio.UploadInfo, error) {
	...
	// 1. Sanitizar a chave do objeto (remover barras duplas e barras iniciais)
	objectName = strings.TrimPrefix(objectName, "/")
	for strings.Contains(objectName, "//") {
		objectName = strings.ReplaceAll(objectName, "//", "/")
	}

	// 2. Normalizar o mapa de metadados para garantir que nenhuma chave venha com prefixos duplicados ou inválidos
	cleanMetadata := make(map[string]string)
	for k, v := range userMetadata {
		// Remove qualquer prefixo 'x-amz-meta-' caso ainda venha do caller
		cleanKey := strings.TrimPrefix(strings.ToLower(k), "x-amz-meta-")
		cleanMetadata[cleanKey] = v
	}

	opts := minio.PutObjectOptions{
		ContentType:  contentType,
		UserMetadata: cleanMetadata,
	}
