...
func JWTAuthMiddleware(cfg *config.Config, logger *zap.SugaredLogger) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		// Injeta também sob a chave genérica "user" para compatibilidade com getUserClaims
		c.Set("user", claims)
...
func validateTokenLocally(tokenString, secretKey, expectedIssuer string) (*model.CustomJWTClaims, error) {
	...
	// Normaliza a Role principal a partir do array Roles se necessário (ex: "roles": ["admin"])
	...
	// Normalizar ID do usuário se vindo do Subject ou outra fonte
